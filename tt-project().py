import pygame, sys, openpyxl, random
...
def make_tt():
    global grpno_1,grpno_2,grpno_3,grpno_4,grpno_5,grpno_6,grpno_7,grpno_8
    grpno_ = [grpno_1,grpno_2,grpno_3,grpno_4,grpno_5,grpno_6,grpno_7,grpno_8]
    finaltt = []
    for grps in range(0,8):
        wb.active = grps+1
        totaltt = [] 
        if grpno_[grps] > 0 and wb.active.max_row > 1 :
            for classes in range(0,grpno_[grps]):  # duplicates the class as many times as grp count defined on gui
                subsdetail = []
                for i in range(1,wb.active.max_row+1): # selects rows
                    currentsub = []
                    for j in range(1,wb.active.max_column+1): # selects the columns
                        cell_obj = wb.active.cell(row = i, column = j).value
                        currentsub.append(cell_obj)
                    subsdetail.append(currentsub)
                totaltt.append(subsdetail)
            finaltt.append(totaltt) #######   Format of nested list  =    total > grp > classes > [subject,weekly,dailylimit]
    wb.save("tt_data.xlsx") #saves the file (for no reason)
    unorderedtt = []
    orderedtt = []
    classcount = 0
    for i in finaltt:
        for j in i:
            classcount +=1
            unorderedtt.append(j) #######   Format of nested list  =    total > classes > [subject,weekly,dailylimit]
    for i in range(0,classcount):
        orderedtt.append([['','','','','','','',''],['','','','','','','',''],['','','','','','','',''],['','','','','','','',''],['','','','','','','','']])

    wb.active = 0
    sublimcopy = {} # subject max limit of teachers
    for i in range(1,wb.active.max_row+1): # selects rows
        sublimcopy[wb.active.cell(row = i, column = 1).value] = wb.active.cell(row = i, column = 2).value
    sublim = sublimcopy.copy()
    for l in unorderedtt:
        x = unorderedtt.index(l)
        l = sorted([i[::-1] for i in l])
        l = [i[::-1] for i in l]
        l.reverse()
        unorderedtt[x] = l

    #START OF ORDERING THE TIMETABLE   
    for section in range(0,len(unorderedtt)):
        for subject in unorderedtt[section]:
            if subject[1] >=5:
                for day in range(0,5):
                    ...
                    while True:
                        ...
                        if orderedtt[section][day][period] != '':
                            periods.remove(period)
                            continue
                        if sublim[subject[0]] > 2 and section > 2:#################
                            for ---- count how many same periods and then compare
                            if orderedtt[i][day][period] != subject[0]:
                                periods.remove(period)
                                continue###################
                        for i in range(0,len(orderedtt)):
                            if orderedtt[i][day][period] == subject[0]:
                                sublim[subject[0]] -= 1
                                if sublim[subject[0]] == 0:
                                    sublim[subject[0]] += 1
                                    periods.remove(period)
                                    errored = True
                                    break
                        if errored == True:
                            continue
                        orderedtt[section][day][period] = subject[0]
                        subject[1] -= 1
                        break
    ...
    for section in range(0,len(unorderedtt)):
        for subject in unorderedtt[section]:
            while subject[1] < 5 and subject[1] > 0: 
                dayerror = False
                days= [0,1,2,3,4]
                while True:
                    day = random.choice(days)
                    if '' not in orderedtt[section][day]:
                        days.remove(day)
                        print('error no space',subject[0],section,day)
                        continue
                    if orderedtt[section][day].count(subject[0]) > subject[2]:
                        days.remove(day)
                        print('max day limit reached',subject[0],section,day)
                        continue
                    print('passed',subject[0],section,day)
                    periods = [0,1,2,3,4,5,6,7]
                    sublim = sublim.copy()
                    while True:
                        errored = False
                        ...
                        break
                    if dayerror==True:
                        print('error not eligible anywhere',subject[0],section,day)
                        days.remove(day)
                        continue
                    break
