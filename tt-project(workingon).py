...
def make_tt():
    ...
    wb.active = 0
    sublim = {} # subject max limit of teachers
    for i in range(1,wb.active.max_row+1): # selects rows
        sublim[wb.active.cell(row = i, column = 1).value] = wb.active.cell(row = i, column = 2).value

    #START OF ORDERING THE TIMETABLE    
    for section in unorderedtt :
        days = [0,1,2,3,4]
        print('ok till here')
        for subject in section :
            subject[1] = int(subject[1])
            if subject[1] >= 5 : ##########  other - else : day is random, if '' in the day, then copy paste while True block after it.
                for day in range(0,5):
                    periods = [0,1,2,3,4,5,6,7]
                    print(periods,'>5')
                    while True:
                        period = random.choice(periods)
                        if orderedtt[unorderedtt.index(section)][day][period] != '':
                        ...
                        for i in range(0,len(orderedtt)):
                            if orderedtt[i][day][period] == subject[0]:
                                repeat +=1
                        if repeat > sublim[subject[0]]:
                            periods.remove(period)
                            continue
                        orderedtt[unorderedtt.index(section)][day][period] = subject[0]
                        subject[1] -= 1
                            ###########TO BE CONTINUED - PROGRESS - CHECKED FOR EMPTY PLACE - repeat subject on other day under progress
            else:
                print(periods,'<5')
                day = random.choice(days)
                count = orderedtt[unorderedtt.index(section)][day].count(subject[0])
                if count > int(subject[2]) or '' not in orderedtt[unorderedtt.index(section)][day] :
                    days.remove(day)
                    continue
                else:
                    periods = [0,1,2,3,4,5,6,7]
