...
def make_tt():
    ...
    for grps in range(0,8):
        ...
        if grpno_[grps] > 0 and wb.active.max_row > 1 :
            for classes in range(0,grpno_[grps]):
                subsdetail = []
                for i in range(1,wb.active.max_row+1):
                    currentsub = []
                    for j in range(1,wb.active.max_column+1):
            ...
            finaltt.append(totaltt)
    print(finaltt) #######   Format of nested list  =    total > grp > no. of classes > [subject,weekly,dailylimit]
